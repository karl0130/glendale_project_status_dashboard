...
남고, `데이터 & 아카이브` 화면에서 JSON 을 내려받아 `data/` 에 커밋하는 옛 방식으로
반영할 수 있다.
...
월말 보고용 단일 파일이나 구글 밖 백업이 필요하면 `데이터 & 아카이브` 화면에서 전체
스냅샷을 내려받는다.
...
그래서 시트 연결 상태에서는 내보내기-커밋 UI 자체를 띄우지 않는다.
...
4. 대시보드에서 구글 로그인 → `데이터 & 아카이브` → **시트 초기화** 를 한 번 누른다.
